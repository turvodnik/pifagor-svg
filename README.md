...
- Возле основных кнопок и настроек есть русские подсказки через значок `i`.
...
- Пользовательские профили могут задавать `px`, `em`, `rem`, `%`, stroke/fill, толщину stroke, удаление фона, очистку `defs`, safe `clipPath`, `style`, `id`, пустых групп, простых `<use>` и внутренних `url(#...)`.
